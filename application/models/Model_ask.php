...
class Model_ask extends CI_Model {
	...
	public function Get() {
		...
		$this->db->where('tbl_ask.ask_answered','0');

		$this->db->join('tbl_ask_categories','tbl_ask_categories.categories_ask_id=tbl_ask.categories_ask_id');
	...
	function getDataById($ask_id) {
		$this->db->select('*');
		$this->db->from('tbl_ask');
		$this->db->where('tbl_ask.ask_id', $ask_id);
